...
import { ArrowLeft, Loader2, Save, DollarSign, CheckCircle } from 'lucide-react';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', paddingBottom: '40px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            <h1 style={{ fontSize: '24px', fontWeight: 700, color: '#1a1a1a' }}>Distribution: {collection.tips_id}</h1>
            <div style={{ fontSize: '14px', color: 'var(--text-muted)' }}>{collection.branch} • {new Date(collection.date_from).toLocaleDateString()} - {new Date(collection.date_to).toLocaleDateString()}</div>
          </div>
        </div>
