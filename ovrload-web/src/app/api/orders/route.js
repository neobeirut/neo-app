...
async function sendAutomatedOrderWhatsAppMessages({ orderId, orderNumber, branchId }) {
  ...
  try {
    ...
    try {
      ...
      await sendWhatsAppFreeForm(branchPhoneE164, ovrloadMsgText).catch(async () => {
        // Fallback template sending
        await sendNewOrderWhatsApp({ orderId, orderNumber, branchId });
      });
...
export async function POST(request) {
  ...
  try {
    ...
    // Send automated Infobip WhatsApp notifications to OVR LOAD and Client (non-blocking)
