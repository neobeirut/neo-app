...
const SHARED_ALIASES = {
  ...
  '@sentry/react-native': path.resolve(__dirname, './polyfills/shared/sentry-react-native/index.js'),
};
...
config.resolver.resolveRequest = (context, moduleName, platform) => {
  try {
    ...
    if (
      normalizedOrigin.startsWith(`${normalizedDir}/polyfills/native`) ||
      normalizedOrigin.startsWith(`${normalizedDir}/polyfills/web`) ||
      normalizedOrigin.startsWith(`${normalizedDir}/polyfills/shared`)
    ) {
    ...
    if (moduleName === 'react-native-purchases') {
      console.log('RESOLVING METRO ALIAS FOR react-native-purchases:', {
        ...
        CREATE_ENV: process.env.EXPO_PUBLIC_CREATE_ENV,
        willAlias: process.env.NODE_ENV !== 'production',
        targetPath: DEV_ONLY_NATIVE_ALIASES[moduleName]
      });
    }
    if (
...
// config.fileMapCacheDirectory removed - not a valid Metro config option
config.reporter = {
  ...config.reporter,
  update: (event) => {
