...
export default function ScannerScreen() {
  ...
  useEffect(() => {
    console.log("[Scanner] Mount - Loaded Wellness-Restored Version (2026-06-23)");
  ...
  const handleCapture = async () => {
    ...
    try {
      ...
      // Compress and resize the captured photo (smaller resolution for much faster transfer/AI)
      ...
      // Send to backend
      const localUrl = "/api/nutrition/analyze";
      console.log("[Scanner] Sending request to resolved URL:", localUrl);
      ...
      {analysisResult ? (
        ...
        <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: insets.bottom + 20 }]}>
          <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.separator }]}>
            ...
            {analysisResult.is_wellness_bowl === false ? (
              // Unsupported food alert
              <View style={{ alignItems: "center", paddingVertical: 12 }}>
                <AlertCircle size={48} color="#EF4444" style={{ marginBottom: 16 }} />
                <Text style={[styles.wellnessTitle, { color: colors.text, textAlign: "center" }]}>
                  Item Not Supported
                </Text>
                
                <View style={[styles.descriptionBox, { backgroundColor: colors.background, borderLeftColor: "#EF4444", width: "100%", marginVertical: 16 }]}>
                  <Text style={[styles.descriptionText, { color: colors.text, fontSize: 15, fontStyle: "normal", textAlign: "center" }]}>
                    {analysisResult.unsupported_message || "This food item is not supported by the wellness scanner. Enjoy your meal!"}
                  </Text>
                </View>

                {/* Scan Another Button */}
                <TouchableOpacity style={[styles.scanAgainBtn, { width: "100%" }]} onPress={handleReset}>
                  <RefreshCw size={18} color="#ffffff" style={{ marginRight: 8 }} />
                  <Text style={styles.scanAgainText}>Scan Another Plate</Text>
                </TouchableOpacity>

                {/* View History Button if authenticated */}
                {isAuthenticated && (
                  <TouchableOpacity
                    style={{
                  ...
                  >
                    ...
                    >
                      View Wellness Bar History
                    </Text>
                  </TouchableOpacity>
                )}
              </View>
            ) : (
              // Supported Wellness Bowl Card
              <>
                <Text style={[styles.wellnessTitle, { color: colors.text }]}>{analysisResult.food_name || "Your Wellness Plate"}</Text>
