...
const PUBLISHED_WEB_BASE_URL = "https://ovrload-backend-production.up.railway.app";
...
function getApiBaseUrl() {
  ...
  // Use local backend server port ONLY if explicitly requested via environment flag
  if (process.env.EXPO_PUBLIC_USE_LOCAL_BACKEND === "true" && __DEV__ && Platform.OS !== "web") {
  ...
  const chosen = candidate && candidate.length > 0 ? candidate : PUBLISHED_WEB_BASE_URL;
